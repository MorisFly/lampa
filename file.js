...
{ show: 'housewives', difficulty: 'hard', question: 'Как называется ключевая тема сериала?', answers: ['Секреты пригородной жизни', 'Криминальные расследования', 'Фантастика', 'Политика'], correct: 0 }
    ...
    // Массив элементов для ТВ-фокуса на текущем экране
    ...
    function init() {
        ...
        // Регистрируем ТВ-контроллер в системе Лампы
    ...
    // ==========================================
    // ТЕЛЕВИЗИОННОЕ УПРАВЛЕНИЕ (Lampa.Controller)
    // ==========================================
    function registerTvController() {
        Lampa.Controller.add('quiz_gate', {
            toggle: function () {
                // Переключение фокуса на наш плагин
            },
            up: function () {
            ...
            back: function () {
                // Если нажать НАЗАД на пульте, симулируем нажатие экранной кнопки возврата (если она есть)
        ...
        // Жестко перехватываем фокус на себя
        Lampa.Controller.toggle('quiz_gate');
    }

    // Обновление списка интерактивных элементов на экране
    ...
    // Визуальное выделение элемента при перемещении пульта
    function applyTvFocus() {
        ...
        if (focusableElements[focusIndex]) {
            ...
            // Автопрокрутка, если контент не влезает на старых ТВ
            if (currentEl.length) {
    ...
    function renderIntroScreen() {
        ...
        content.html(`
            ...
            <p>Вам предстоит ответить на <b>5 случайных вопросов</b>. Для успешного входа нужно дать минимум <b>3 правильных ответа</b>.</p>
    ...
    function startQuizLogic() {
        ...
        if (pool.length < 5) pool = QUESTIONS_BASE.filter(q => q.show === gameState.selectedShow);

        gameState.quizQuestions = pool.sort(() => 0.5 - Math.random()).slice(0, 5);
    ...
    // 4. Экран вопроса (Оптимизирован под список)
    function renderQuestionScreen() {
        ...
        content.html(`
            <div class="quiz-header-nav">
                ...
                <span class="quiz-step-indicator">Вопрос ${gameState.currentIndex + 1} из 5</span>
            </div>
        ...
        updateFocusMap();
        // На экране вопроса по дефолту ставим фокус на первый вариант ответа (индекс 1, так как индекс 0 — кнопка сброса)
        focusIndex = 1;
        applyTvFocus();
    }
    ...
    function renderFinalResultScreen() {
        const content = $('#quiz-gate-content');
        const isWin = gameState.correctCount >= 3;

        if (isWin) {
            content.html(`
                <h2>Поздравляем! 🎉</h2>
                <p class="final-status-msg">Вы успешно прошли верификацию фаната!</p>
                <div class="final-score-box">Результат: ${gameState.correctCount} из 5 верных</div>
            ...
            content.find('.unlock-lampa-btn').on('click', function() {
                ...
                // Возвращаем стандартное управление Лампы на главный экран
            ...
            content.html(`
                ...
                <div class="quiz-options-list">
                    <button class="quiz-btn primary-btn quiz-focusable retry-same">Попробовать снова (другие вопросы)</button>
    ...
    function injectStyles() {
        const css = `
            ...
            /* ЭФФЕКТ ТВ ФОКУСА (Пульт на кнопке) */
            .quiz-btn.focus { 
